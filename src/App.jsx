...
function App() {
  ...
  try {
    ...
    // 📊 Auftrag
    pdf.text("Auftragsdaten:", 10, y);
    y += 8;

    pdf.text(`Nummer: ${a.nummer}`, 10, y);
    y += 6;

    pdf.text(`Fahrer: ${a.fahrer}`, 10, y);
    y += 6;

    pdf.text(`Material: ${a.material}`, 10, y);
    y += 10;

    // 📍 Adresse
    pdf.text("Adresse:", 10, y);
    y += 8;

    pdf.text(`${a.strasse}`, 10, y);
    y += 6;

    pdf.text(`${a.plzOrt}`, 10, y);
    y += 10;
    ...
    pdf.text(`${a.status}`, 10, y);
    y += 10;

    // 🕒 Erledigt
    pdf.text("Erledigt am:", 10, y);
    y += 8;

    pdf.text(formatDate(a.zeitErledigt), 10, y);
    y += 10;

    // 📍 GPS
    if (a.gps?.lat && a.gps?.lng) {
      pdf.text("GPS Koordinaten:", 10, y);
      y += 8;

      pdf.text(`${a.gps.lat}, ${a.gps.lng}`, 10, y);
      y += 10;
    }
    ...
    pdf.line(10, y, 100, y);
    y += 5;
    pdf.text("Unterschrift Fahrer", 10, y);
