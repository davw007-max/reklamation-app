...
app.put("/auftraege/:id", async (req, res) => {
  try {
    // ✅ NEU: 'statusUpdate' und 'bild' aus dem Body mit auslesen
    ...
    // ✅ Wenn ein Fahrer mitgeschickt wurde (Dispo ändert Fahrer im Dropdown)
    if (fahrer !== undefined) {
    ...
    // --- Ab hier läuft die Status-Logik (Erledigt ODER Fehlanfahrt) ---
    ...
    // Den Status in der Datenbank anpassen
    if (wirdErledigt) {
      auftrag.status = "erledigt";
    } else if (wirdFehlanfahrt) {
      auftrag.status = "fehlanfahrt";
    } else if (!wirdErledigt && !wirdFehlanfahrt && !statusUpdate) {
      auftrag.status = "offen"; // Für den alten Toggle-Button (zurück auf offen)
    }

    // ✅ PDF generieren (egal ob erfolgreich erledigt oder Fehlanfahrt)
    if (wirdErledigt || wirdFehlanfahrt) {
      ...
      doc.on("end", async () => {
        ...
        await transporter.sendMail({
          ...
          // ✅ Betreff dynamisch: Steht jetzt entweder ERLEDIGT oder FEHLANFAHRT drin
        ...
        console.log("📧 Mail versendet:", auftrag.status.toUpperCase());
      });
      ...
      doc.font("Helvetica");
      // ✅ Zeigt "ERLEDIGT" oder "FEHLANFAHRT" an
      doc.text(`Status: ${auftrag.status.toUpperCase()}`, 40, y); y += 15;
      doc.text(
        `Gemeldet am: ${new Date(auftrag.zeitErledigt).toLocaleString("de-DE")}`,
        40,
        y
      );
      ...
      // ✅ NEU: BILD INS PDF DRUCKEN WENN VORHANDEN
      if (bild) {
        doc.font("Helvetica-Bold").text("Beweisfoto (Fehlanfahrt):", 40, y);
        y += 20;
        
        try {
          const base64Data = bild.split(",")[1];
          // Bildgröße anpassen, damit es aufs Blatt passt
          doc.image(Buffer.from(base64Data, "base64"), 40, y, { width: 250 });
          y += 200; // Platz nach unten lassen, damit das GPS nicht überschrieben wird
        } catch(e) {
          doc.font("Helvetica").text("Fehler beim Laden des Beweisfotos.", 40, y);
          y += 20;
        }
      }
